...
export default function ClientsPage() {
  ...
  // Computation for the History tab: lists all clients that have remarks (even if 0 active debts in import)
  const historicalClients = useMemo(() => {
    ...
    return allKnownClientNames
      ...
      .filter(item => {
        if (item.remarks.length === 0 && !item.isActiveInImport) return false;
      ...
      .sort((a, b) => {
        // Put inactive/archived clients with remarks at top or sort by last remark date
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
            ...
            <div className="flex items-center justify-between flex-wrap gap-4">
              ...
              <div className="flex items-center gap-2 pt-3 border-t border-slate-100 w-full">
                ...
                >
                  ...
                  <span>Créances Import Actuel</span>
        ...
        <main className="flex-1 overflow-y-auto p-4 md:p-8 bg-slate-50/50">
          <div className="max-w-7xl mx-auto space-y-4 md:space-y-6">
            ...
            {activeTab === 'history' ? (
              <div className="space-y-4">
                <div className="bg-gradient-to-r from-slate-900 via-blue-900 to-indigo-950 text-white p-6 rounded-3xl shadow-xl flex items-center justify-between flex-wrap gap-4">
                  <div>
                    <div className="flex items-center gap-2 mb-1">
                      <History className="h-5 w-5 text-blue-400" />
                      <h2 className="text-lg font-black tracking-tight">Historique & Archives des Remarques Clients</h2>
                    </div>
                    <p className="text-xs text-blue-200 font-medium max-w-2xl">
                      Consultez l'historique complet de toutes les remarques, promesses et relances enregistrées pour l'ensemble des clients, y compris ceux sans créance active dans l'import actuel.
                    </p>
                  </div>
                  <Badge variant="outline" className="border-blue-400/30 bg-blue-500/20 text-blue-100 font-bold px-3.5 py-1.5 text-xs rounded-xl">
                ...
                {historicalClients.length === 0 ? (
                  <Card className="p-8 text-center bg-white rounded-3xl border-slate-200">
                    ...
                    <p className="text-sm font-bold text-slate-600">Aucun historique client trouvé</p>
                        ...
                        <div>
                          <div className="flex items-start justify-between gap-3 mb-2">
                            ...
                            <Button
                              size="sm"
                              variant="outline"
                              className="rounded-xl border-blue-200 text-blue-700 hover:bg-blue-50 text-xs font-bold gap-1.5 flex-shrink-0"
                              onClick={(e) => handleOpenRemarkModal(e, item.clientName)}
                            >
                              <MessageSquare className="h-3.5 w-3.5" />
                              Voir / Ajouter ({item.remarks.length})
                            </Button>
                          </div>
                    ...
                    <div className="px-4 md:px-8 pb-4 md:pb-8 pt-0 border-t border-slate-50">
                      ...
                      <div className="hidden md:block bg-white rounded-2xl md:rounded-[24px] overflow-x-auto border border-slate-200 shadow-inner scrollbar-hide">
                        <Table>
                          ...
                          <TableBody>
                            {clientDebts.map((debt: ClientDebt, i: number) => {
                                  ...
                                  <TableCell className="font-mono text-xs md:sm font-bold text-slate-700 py-3 md:py-5 px-4 md:px-6">
                                    <div className="flex items-center gap-2">
                                      ...
                                      {debt.isManualContentieux && (
                                        <span title="Modifié manuellement (ne suit pas les règles d'âge)">
